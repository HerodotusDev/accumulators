import CoreMMR, { IStore, PrecomputationMMR } from "@herodotus_dev/mmr-core";
...
describe("precomputation", () => {
  ...
  beforeAll(async () => {
    const client = new DynamoDB.DocumentClient({ region: "eu-west-2" });
    store = new DynamoDbStore(client, "mmrs");
  });
